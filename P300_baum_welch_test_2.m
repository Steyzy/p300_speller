function [answer,staticResults1,NLPResults1] = P300_baum_welch_test_2(szFile,trainData1, allStim1, parameters1, labels1, coeff, feaSelector,subject_index,map,TotalTrials)
...
for i=TotalSet
    ...
    [~,~,~, all_coeffs, all_feaSelectors,~,~,~,~,~] = P300_baum_welch(trainData1, allStim1, parameters1, labels1, coeff, feaSelector, map, subject_index,i);
    coeff1=all_coeffs{subject_index}{length(all_coeffs{subject_index})};
    feaSelector1=all_feaSelectors{subject_index}{length(all_feaSelectors{subject_index})};
